...
object ShaderInstance {
    ...
    val fragmentShader
        get() = oldFragmentShader

    // language=GLSL
    const val oldFragmentShader = """
...
float focus = 1.0;
  mat2 baseVector = mat2(vec2((0.5 - focus) * mouse/threshold - (mouse/threshold)/2.0) , 
                       vec2((0.5 - focus) * mouse/threshold + (mouse/threshold)/2.0));
  ...
  for(float i = 0.0; i < MAXSTEPS; ++i) {
    ...
    #endif
      float depth = 1.0 - texture2D(image1, vpos * vec2(1.0, -1.0) + vec2(0, 1.0)).r;
    ...
    gl_FragColor = texture2D(image0, posSum / confidenceSum);
  #endif
