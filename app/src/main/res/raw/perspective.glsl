...
float scale = 1;
vec2 scale2 = vec2(scale * min(1.0, 1.0 / aspect), scale * min(1.0, aspect)) * vec2(1.0, -1.0) * vec2(ENLARGE);
// mat2 baseVector = mat2(vec2(-focus * offset) * scale2, vec2(offset - focus * offset) * scale2);
mat2 baseVector = mat2(vec2((0.5 - focus) * mouse/threshold - (mouse/threshold)/2.0) * scale2,
vec2((0.5 - focus) * mouse/threshold + (mouse/threshold)/2.0) * scale2);
...
void main(void) {
    ...
    for(float i = 0.0; i < MAXSTEPS; ++i) {
        ...
        if (dpos >= vectorCutoff && confidenceSum < CONFIDENCE_MAX) {
            #endif
            float depth = 1.0 - texture2D(image1, vpos * vec2(1, -1) + vec2(0, 1)).r;
            ...
            * step(1.5, steps - i);
            ...
            if (confidence > 0.0) {
                ...
                #ifdef COLORAVG
                colSum += texture2D(image0, vpos CORRECTION_MATH) * confidence;
                #else
    ...
    gl_FragColor = texture2D(image1, posSum / confidenceSum);
    #endif
